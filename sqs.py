...
def get_queues(sqs):
    ...
    try:
        ...
        if error_code == "AWS.SimpleQueueService.NonExistentQueue":
            sqs.create_queue(
                QueueName=sqs_name, Attributes={"ReceiveMessageWaitTimeSeconds": "20"}
            )
            print(
                "\n\033[96m"
            ...
            time.sleep(5)
            print("\033[96m" + " ••• ATTACHING REQUIRED ACCESS POLICIES'" + "\033[0m")
            time.sleep(5)
        ...
        if error_code_ == "AWS.SimpleQueueService.QueueDeletedRecently":
            ...
            for remaining in range(60, 0, -1):
                sys.stdout.write("\r")
                sys.stdout.write(
                    "\033[93m" + " ••• {:2d} SECONDS REMAINING".format(remaining)
                )
                sys.stdout.flush()
                time.sleep(1)

            sys.stdout.write(
                "\033[92m" + "\r ••• SQS QUEUE SCRIPT LAUNCHED                  \n"
            )
            ...
            print(
                "\n\033[96m" + " ••• CREATING SQS QUEUE 'cpd-sqs-s1903342'" + "\033[0m"
            )

            time.sleep(5)

            print("\n\033[96m" + " ••• ATTACHING REQUIRED ACCESS POLICIES" + "\033[0m")
